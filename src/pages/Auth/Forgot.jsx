...
const ForgotPassword = () => {
    ...
    async function requestOTP({ email }) {
        ...
        if (data.success) {
            setAlert(['success', `Code OTP nya adalah : ${data.data.code_otp}`])
        } else {
            setAlert(['fail', data.message])
        }
    }
                ...
                <div className="w-full flex flex-col justify-center items-center gap-4 p-10 flex-3 text-(--color-secondary)">
                    <form className="flex w-full flex-col justify-center gap-4 p-4 flex-3" onSubmit={handleSubmit(forgot)}>
                        ...
                        <div className="flex flex-col justify-center gap-4">
                            <Input type="email" {...register("email")} labelName="Email" icon={mail_icon} placeholder="Enter email address" className="flex-4" required ref={email} />
                            <div className="flex items-center gap-4">
                                <button className="bg-(--color-primary) text-black p-4 rounded cursor-pointer" onClick={()=>{requestOTP({email: email.current.value})}}>Request OTP</button>
                            </div>
                        </div>
